...
if __name__ == "__main__":
    ...
    vae = SD_VAE('/data/checkpoints/stabilityai/sd-vae-ft-ema/vae-ft-ema-560000-ema-pruned.safetensors')
    vae.load()
    print(vae.model)
    print(f"SD_VAE Parameters: {sum(p.numel() for p in vae.model.parameters()) / 1e6:.2f}M")
    print(f"SD_VAE Encoder Parameters: {sum(p.numel() for p in vae.model.encoder.parameters()) / 1e6:.2f}M")
    print(f"SD_VAE Decoder Parameters: {sum(p.numel() for p in vae.model.decoder.parameters()) / 1e6:.2f}M")
    ...
    Image.fromarray(out).save('sdvae.jpg')