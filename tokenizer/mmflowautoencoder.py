...
class MMFlowAutoencoderKL(nn.Module):
    def __init__(
            self, 
            embed_dim, 
            ch_mult, 
        ...
        self.y_embedder = PatchEmbed(input_size // patch_size, 1, self.embed_dim, hidden_size, bias=True)
    ...
    def forward(self, x, t=None, y=None, **kwargs):
        ...
        for idx, block in enumerate(self.blocks):
            if self.use_checkpoint:
                x = checkpoint(block, x, t, self.feat_rope, use_reentrant=True)
            else:
                x = block(x, t, self.feat_rope)
...
def MMFlowVAVAE_S(**kwargs):
    return MMFlowAutoencoderKL(
        embed_dim=32, patch_size=16, ch_mult=(1, 1, 2, 2, 4), model_type='vavae', 
        depth=12, hidden_size=384, num_heads=6, **kwargs
        )

def MMFlowSDVAE_S(**kwargs):
    return MMFlowAutoencoderKL(
        embed_dim=4, patch_size=8, ch_mult=(1, 2, 4, 4), model_type='sdvae', 
        depth=12, hidden_size=384, num_heads=6, **kwargs
        )

def MMFlowMARVAE_S(**kwargs):
    return MMFlowAutoencoderKL(
        embed_dim=16, patch_size=16, ch_mult=(1, 1, 2, 2, 4), model_type='marvae', 
        depth=12, hidden_size=384, num_heads=6, **kwargs
        )

MMFlowVAE_models = {
    'MMFlowVAVAE-S':  MMFlowVAVAE_S,  
    'MMFlowSDVAE-S':  MMFlowSDVAE_S,  
    'MMFlowMARVAE-S':  MMFlowMARVAE_S,  
}
...
if __name__ == '__main__':
    ...
    config_path = 'configs/mmflowsdvae_100kx1024.yaml'
    ...
    in_channels = train_config['vae']['in_chans']
    model = MMFlowVAE_models[train_config['vae']['vae_type']](
        input_size=train_config['data']['image_size'],     
        in_channels=in_channels,
        use_qknorm=train_config['vae']['use_qknorm'],
        use_swiglu=train_config['vae']['use_swiglu'] if 'use_swiglu' in train_config['vae'] else False,
        use_rope=train_config['vae']['use_rope'] if 'use_rope' in train_config['vae'] else False,
        use_rmsnorm=train_config['vae']['use_rmsnorm'] if 'use_rmsnorm' in train_config['vae'] else False,
        use_checkpoint=train_config['vae']['use_checkpoint'] if 'use_checkpoint' in train_config['vae'] else False,
