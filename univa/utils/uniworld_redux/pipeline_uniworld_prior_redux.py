...
from .modeling_uniworld import ReduxImageEncoder
from .pipeline_output import UniWorldPriorReduxPipelineOutput
...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import UniWorldPriorReduxPipeline, FluxPipeline
        >>> from diffusers.utils import load_image

        >>> device = "cuda"
        >>> dtype = torch.bfloat16

        >>> repo_redux = "LanguageBind/UniWorld-Redux"
        >>> repo_base = "black-forest-labs/FLUX.1-dev"
        >>> pipe_prior_redux = UniWorldPriorReduxPipeline.from_pretrained(repo_redux, torch_dtype=dtype).to(device)
...
RATIO = {
    'any_15ratio': [(2, 1), (1, 2), (16, 9), (9, 16), (7, 6), (6, 7), (7, 5), (5, 7), (5, 4), (4, 5), (4, 3), (3, 4), (3, 2), (2, 3), (1, 1)],
...
class UniWorldPriorReduxPipeline(DiffusionPipeline):
    r"""
    The UniWorld Redux pipeline for image-to-image generation.
    ...
    def __init__(
        ...
        self.feature_extractor.size = {"height": 1024, "width": 1024}
    ...
    def encode_image(self, image, device, num_images_per_prompt):
        ...
        image_enc_hidden_states = rearrange(
            image_enc_hidden_states,
            "(b h2 w2) (h w) c -> b (h2 h) (w2 w) c",
            h2=2,
            w2=2,
            h=height // self.image_encoder.config.patch_size // 2,
            w=width // self.image_encoder.config.patch_size // 2,
        )
    ...
    def __call__(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`~pipelines.uniworld.UniWorldPriorReduxPipelineOutput`] instead of a plain tuple.

        Examples:

        Returns:
            [`~pipelines.uniworld.UniWorldPriorReduxPipelineOutput`] or `tuple`:
            [`~pipelines.uniworld.UniWorldPriorReduxPipelineOutput`] if `return_dict` is True, otherwise a `tuple`. When
        ...
        if image is not None and isinstance(image, Image.Image):
            ...
            gen_height, gen_width = dynamic_resize(
                height, width, 
                anyres=self.image_embedder.config.anyres, 
        ...
        return UniWorldPriorReduxPipelineOutput(
            prompt_embeds=prompt_embeds, 
            pooled_prompt_embeds=pooled_prompt_embeds, 
            height=gen_height, 
            width=gen_width, 
