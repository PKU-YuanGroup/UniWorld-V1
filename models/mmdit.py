...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rope=None, rope_begin=0) -> torch.Tensor:
        B, N, C = x.shape
        ...
        if rope is not None:
            q = rope(q) if rope_begin == 0 else torch.concat([q[:, :, :rope_begin], rope(q[:, :, rope_begin:])], dim=2)
            k = rope(k) if rope_begin == 0 else torch.concat([k[:, :, :rope_begin], rope(k[:, :, rope_begin:])], dim=2)
        if self.fused_attn:
...
def MMDiT_S_1(**kwargs):
    return MMDiT(depth=12, hidden_size=384, patch_size=1, num_heads=6, **kwargs)

def MMDiT_S_2(**kwargs):
    return MMDiT(depth=12, hidden_size=384, patch_size=2, num_heads=6, **kwargs)

def MMDiT_XL_1(**kwargs):
    return MMDiT(depth=28, hidden_size=1152, patch_size=1, num_heads=16, **kwargs)
...
def MMDiT_B_1(**kwargs):
    return MMDiT(depth=12, hidden_size=768, patch_size=1, num_heads=12, **kwargs)

def MMDiT_B_2(**kwargs):
    return MMDiT(depth=12, hidden_size=768, patch_size=2, num_heads=12, **kwargs)

def MMDiT_1p0B_1(**kwargs):
    return MMDiT(depth=24, hidden_size=1536, patch_size=1, num_heads=24, **kwargs)

def MMDiT_1p0B_2(**kwargs):
    return MMDiT(depth=24, hidden_size=1536, patch_size=2, num_heads=24, **kwargs)

def MMDiT_1p6B_1(**kwargs):
    return MMDiT(depth=28, hidden_size=1792, patch_size=1, num_heads=28, **kwargs)

def MMDiT_1p6B_2(**kwargs):
    return MMDiT(depth=28, hidden_size=1792, patch_size=2, num_heads=28, **kwargs)

MMDiT_models = {
    'MMDiT-S/1':  MMDiT_S_1, 'MMDiT-S/2':  MMDiT_S_2, 
    'MMDiT-B/1': MMDiT_B_1, 'MMDiT-B/2': MMDiT_B_2,
    'MMDiT-L/2': MMDiT_L_2,
    'MMDiT-XL/1': MMDiT_XL_1, 'MMDiT-XL/2': MMDiT_XL_2,
    'MMDiT-1p0B/1': MMDiT_1p0B_1, 'MMDiT-1p0B/2': MMDiT_1p0B_2,
    'MMDiT-1p6B/1': MMDiT_1p6B_1, 'MMDiT-1p6B/2': MMDiT_1p6B_2,
}
...
if __name__ == '__main__':
    ...
    model = MMDiT_models[train_config['model']['model_type']](
    ...
    bsz = 256
    x = torch.rand(bsz, in_channels, latent_size, latent_size).cuda(7)
    t = torch.rand(bsz).cuda()
    y = torch.randint(0, 1000, (bsz, )).cuda(7)
