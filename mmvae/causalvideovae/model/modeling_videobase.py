...
class VideoBaseAE(ModelMixin, ConfigMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ckpt_files = os.listdir(pretrained_model_name_or_path)
        if ckpt_files:
            # Adapt to checkpoint
            dirs = [d for d in ckpt_files if 'checkpoint' in d]
            dirs = sorted(dirs, key=lambda x: int(x.split("-")[1].split('.')[0]))
            last_ckpt_file = os.path.join(pretrained_model_name_or_path, dirs[-1])
