...
class DataCollator:
    def __call__(self, instances: List[Dict]) -> Dict:
        siglip_pixel_values = torch.stack([instance["siglip_pixel_values"][0] for instance in instances])
...
class JointModel(nn.Module):
    def __init__(self, args, lvlm_model, siglip_model):
        super().__init__()
        self.args = args
    ...
    def drop_lora(self, drop_lora_rate=None):
        ...
        for module in self.siglip_model.modules():
            if hasattr(module, "r") and hasattr(module, "lora_alpha"):
                do_drop = drops[index]
                if do_drop:
                    module.scaling = {'default': 0.0}
                else:
                    module.scaling = {'default': module.lora_alpha['default'] / module.r['default']}
            if self.args.training_config.drop_lora_layer_wise:
                index += 1
    
    def revert_lora_scaling(self):
        for module in self.siglip_model.modules():
            if hasattr(module, "r") and hasattr(module, "lora_alpha"):
                module.scaling = {'default': module.lora_alpha['default'] / module.r['default']}
                # print(f'revert scaling to {module.scaling}')


    def forward(self, **kwargs):
        siglip_pixel_values = kwargs.pop('siglip_pixel_values')
        empty_t5_prompt_embeds = kwargs.pop('empty_t5_prompt_embeds')
        ...
        if self.args.model_config.lora_r > 0:
            if self.args.training_config.drop_lora_rate > 0.0:
                self.drop_lora()
        ...
        if self.args.dataset_config.height == 1024 and self.args.dataset_config.width == 1024:
            ...
            siglip_hidden_states = rearrange(
                siglip_hidden_states,
                "b (h w) c -> b h w c",
                h=height // self.siglip_model.config.patch_size,
        ...
        if random.random() > 0.0:
            ...
            encoder_hidden_states = torch.cat([siglip_hidden_states, empty_t5_prompt_embeds], dim=1)
...
def main(args: UnivaTrainingDenoiseConfig, attn_implementation='sdpa'):
    ...
    # Load models
    lvlm_model = UnivaDenoiseTower_V1_1.from_pretrained(
    ...
    if args.model_config.lora_r > 0:
        ...
        #     siglip_model._set_gradient_checkpointing()
    if args.training_config.gradient_checkpointing:
                ...
                if isinstance(accelerator.unwrap_model(model).lvlm_model, UnivaDenoiseTower_V1_1) or isinstance(accelerator.unwrap_model(model).siglip_model, SiglipVisionModel):
                    ...
                    if args.model_config.lora_r > 0:
                        if args.training_config.drop_lora_rate > 0.0:
                            accelerator.unwrap_model(model).revert_lora_scaling()
                        accelerator.unwrap_model(model).siglip_model.save_pretrained(
                            os.path.join(output_dir, "siglip_lora"),
                        )
                else:
    ...
    if args.model_config.only_tune_siglip_mlp:
        ...
        if args.model_config.flux_train_layer_idx is not None:
            ...
            for name, module in joint_model.lvlm_model.named_modules():
    ...
    for name, param in joint_model.siglip_model.named_parameters():
        if param.requires_grad:
            trainable_params.append(param)
            trainable_names.append(name)
    if accelerator.is_main_process:
    ...
    dataset = ReduxDataset(
        ...
        image_processor=siglip_processor,
    ...
    empty_t5_prompt_embeds = empty_t5_prompt_embeds.repeat(
        args.dataset_config.batch_size, 1, 1
    )
    empty_pooled_prompt_embeds = empty_pooled_prompt_embeds.repeat(
        args.dataset_config.batch_size, 1
    )
    zero_t5_prompt_embeds = torch.zeros_like(empty_t5_prompt_embeds, device=accelerator.device, dtype=weight_dtype)
    zero_pooled_prompt_embeds = torch.zeros_like(empty_pooled_prompt_embeds, device=accelerator.device, dtype=weight_dtype)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(joint_model):
                ...
                if isinstance(generated_image, list):
                    ...
                    image_seq_len = int(sum([(i.shape[-1] * i.shape[-2]) // 4 for i in unpad_model_input]) / len(unpad_model_input))
                ...
                def apply_flux_schedule_shift(sigmas, noise, image_seq_len):
                ...
                sigmas = apply_flux_schedule_shift(sigmas, noise, image_seq_len)
                ...
                denoiser_kwargs = {
                    "siglip_pixel_values": siglip_pixel_values, 
                    "empty_t5_prompt_embeds": empty_t5_prompt_embeds if random.random() > 0.0 else zero_t5_prompt_embeds,
                ...
                with torch.amp.autocast("cuda", dtype=weight_dtype):
                    model_pred = joint_model(**denoiser_kwargs)
...
def log_validation(
    ...
    title: str = "",
    empty_t5_prompt_embeds: Optional[torch.Tensor] = None, 
    empty_pooled_prompt_embeds: Optional[torch.Tensor] = None, 
):
    ...
    if args.dataset_config.height == 1024 and args.dataset_config.width == 1024:
        siglip_pixel_values = rearrange(
            siglip_pixel_values, "b c (h2 h) (w2 w) -> (b h2 w2) c h w", h2=2, w2=2
        )
    siglip_features = siglip_model(pixel_values=siglip_pixel_values).last_hidden_state
    siglip_features = siglip_features.to(dtype=weight_dtype)
            
    if args.dataset_config.height == 1024 and args.dataset_config.width == 1024:
        siglip_features = rearrange(
            siglip_features,
            "(b h2 w2) (h w) c -> b (h2 h) (w2 w) c",
            h2=2,
            w2=2,
        ...
        siglip_features = rearrange(
            siglip_features,
            "b (h w) c -> b h w c",
            h=height // siglip_model.config.patch_size,
    ...
    if empty_t5_prompt_embeds is None or empty_pooled_prompt_embeds is None:
        ...
        empty_t5_prompt_embeds = empty_t5_prompt_embeds[:1]
        empty_pooled_prompt_embeds = empty_pooled_prompt_embeds[:1]
